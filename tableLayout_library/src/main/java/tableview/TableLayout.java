...
public class TableLayout extends LinearLayout implements TableColumn.Callback {
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            ...
            x_num = typedArray.getIndex(R.styleable.TableLayout_x_num);
            y_num = typedArray.getIndex(R.styleable.TableLayout_y_num);
            text = typedArray.getString(R.styleable.TableLayout_text);
            typedArray.recycle();
        } else {
            tableMode = 0;
        ...
        if (isInEditMode()) {
            String[] content = new String[x_num];
    ...
    public void onClick(float x, float y) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (tableColumn.getRight() >= x) {
                ...
                tableColumn.onClick(x, y);
                if (null != mCallback) {
                    int _x = (int) (x / TableColumn.mTableRowWidth);
                    int _y = (int) (y / TableColumn.mTableRowHeight);
                    mCallback.onResult(_x, _y);
                }
                return;
            }
        }
    ...
    public interface Callback {
        public void onResult(int x, int y);
    }
}
